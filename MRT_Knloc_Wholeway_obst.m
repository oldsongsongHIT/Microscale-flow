...
c2=1./3.;  arf=1.0;
L=ny-1;
obst([0.4*L+1:0.6*L],[0.4*L+1:0.6*L])=1;
Kn_out=0.084; rho_out=1.00; rho_in=1.00;
...
while error>tol
    ...
    [f]=stream(f,cx,cy);
    %Boundary condition:
    [f]=boundary(nx,ny,f,r,rho,rho_in,rho_out);
    [f]=obstc(nx,ny,f,opp,obst,r,L);
    % Calculate rho, u, v
...
toc;
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++
function [f]=boundary(nx,ny,f,r,rho,rho_in,rho_out)
...
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
function [f]=obstc(nx,ny,f,opp,obst,r,L)
% opp2=[8,7,6,5];
% for i=1:nx
%     for j=1:ny
%         if obst(i,j)==1
...
%             end
%             for k=5:8
%                 f(i,j,k)=r(i,j)*f(i,j,opp(k))+(1-r(i,j))*f(i,j,opp2(k-4));
%             end 
%             f(i,j,9) = f(i,j,9);
%         end
%     end
% end
% 
f([0.4*L+1:0.6*L],0.4*L+1,4)= f([0.4*L+1:0.6*L],0.4*L+1,2);
f([0.4*L+1:0.6*L],0.4*L+1,[7,8])= r([0.4*L+1:0.6*L],0.4*L+1).*f([0.4*L+1:0.6*L],0.4*L+1,[5,6])+(1-r([0.4*L+1:0.6*L],0.4*L+1)).*f([0.4*L+1:0.6*L],0.4*L+1,[6,5]);
f([0.4*L+1:0.6*L],0.6*L,2)= f([0.4*L+1:0.6*L],0.6*L,4);
f([0.4*L+1:0.6*L],0.6*L,[5,6])= r([0.4*L+1:0.6*L],0.6*L).*f([0.4*L+1:0.6*L],0.6*L,[7,8])+(1-r([0.4*L+1:0.6*L],0.6*L)).*f([0.4*L+1:0.6*L],0.6*L,[8,7]);
f(0.4*L+1,[0.4*L+1:0.6*L],3)= f(0.4*L+1,[0.4*L+1:0.6*L],1);
f(0.4*L+1,[0.4*L+1:0.6*L],[6,7])= r(0.4*L+1,[0.4*L+1:0.6*L]).*f(0.4*L+1,[0.4*L+1:0.6*L],[8,5])+(1-r(0.4*L+1,[0.4*L+1:0.6*L])).*f(0.4*L+1,[0.4*L+1:0.6*L],[5,8]);
f(0.6*L,[0.4*L+1:0.6*L],1)= f(0.6*L,[0.4*L+1:0.6*L],3);
f(0.6*L,[0.4*L+1:0.6*L],[5,8])= r(0.6*L,[0.4*L+1:0.6*L]).*f(0.6*L,[0.4*L+1:0.6*L],[7,6])+(1-r(0.6*L,[0.4*L+1:0.6*L])).*f(0.6*L,[0.4*L+1:0.6*L],[6,7]);
end
% Collition
...
for i=1:nx
 ...
 for i=1:nx
     for j=1:ny
        m_temp{i,j}=msl{i,j}*m(1:9,:)-mminv*Fi(1:9,:);
        mte=reshape(m_temp{i,j},9,nx,ny);
        for k=1:9
            f2(k,i,j)=f2eq(k,i,j)+f1(k,i,j)-mte(k,i,j);
        end
    end
end
for i=1:nx
...
filename=['F:\Onedrive\OneDrive - stu.hit.edu.cn\LBM_code\date-2\' num2str(count) '-tecplot2d.dat'];
