...
while error>tol
    ...
    % End of streaming
    %Boundary condition:
    [f]=boundary(nx,ny,f,fpre,r,rho,w,cx,cy,c2);
    % Calculate rho, u, v
...
toc;
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++
function [f]=boundary(nx,ny,f,fpre,r,rho,w,cx,cy,c2)
...
for i=1:nx
    %Top
    f(i,ny,4)=fpre(i,ny,2)+2*r*rho(i,ny)*w(4)*(cx(4)*uw+cy(4)*vw)/c2;
    f(i,ny,7)=r*fpre(i,ny,5)+(1-r)*fpre(i,ny,6)+2*r*rho(i,ny)*w(7)*(cx(7)*uw+cy(7)*vw)/c2;
    f(i,ny,8)=r*fpre(i,ny,6)+(1-r)*fpre(i,ny,5)+2*r*rho(i,ny)*w(8)*(cx(8)*uw+cy(8)*vw)/c2;
    %Below
    f(i,1,2)=fpre(i,1,4);
